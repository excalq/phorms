...
abstract class Phorm_Phorm
{
	...
	/**
	 * Print the form fields, but using div tags.
	 * NOTE: This prints help_text FIRST before other elements!
	 *
	 * @author Arthur Ketcham
	 * @param bool $alt decorate alternating rows with css class "phorm_odd_row"
	 * @param string $template passed in html template to wrap the form elements
	 * @return html of the help_text, label, form fields, and validation errors
	 */
	public function as_divs($alt=FALSE, $template='')
	{
		if(empty($template))
		{
			$template[] = '<div class="phorm_row%odd%">';
			$template[] = '<div class="phorm_label">%label%</div>';
			$template[] = '<div class="phorm_field">%field%%errors%</div>';
			$template[] = '<div class="phorm_help_text">%help_text%</div>';
			$template[] = '</div>';
			$template = implode("\n", $template);
		}
		...
		foreach( $this->fields as $name => $field )
		{
			...
			// If no label is set, ignore that part of the template
			$loop_template = $template; // reset each iteration
			if (($field->label(FALSE) == '')) {
				$loop_template = str_replace('<div class="phorm_label">%label%</div>', '', $loop_template);
			}
