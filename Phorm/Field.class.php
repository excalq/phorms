...
abstract class Phorm_Field
{
	...
	private $imported;
	/**
	 * Description text for the field. This is printed ahead of the field HTML.
	 * @var string
	 */
	...
	/**
	 * Is the field a checkbox or radio button? Validation is slightly different for these
	 * @see Checkbox.class.php, and $this->is_valid()
	 * @var boolean
	 */
	 ...
	 * This help text is placed directly after the form field.
	 * (After the input td/div syntactically in the HTML)
	 *
	...
	{
		return $this->description_text.$this->html().$this->help_text.$this->errors();
	}
	...
	public function is_valid($reprocess=false)
	{
		...
		{
			// AK: Checkboxes must be tested with the checked not value attribute
			if ($this->is_checkbox) {
	...
	public function validate_required_field($value)
	{
		...
		} elseif (!is_array($value) && strtolower($value) == '-- select --') {
			// Checking select options - the default is considered blank.
			throw new Phorm_ValidationError('validation_required');
		}
	}
}